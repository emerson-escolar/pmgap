...
ChangeCLtoTypeAForTwoRows:=function(CLrep,CLIntervalrep,ZigzagPathAlgebra,field)
...
if a<b then
for i in [1..b-a] do
Mat_2:=Mat_2*MatricesOfPathAlgebraModule(CLrep)[(size-1)+a+i-1];
od;
fi;
...
if a<b then
for i in [1..b-a] do
Mat_1:=Mat_1*MatricesOfPathAlgebraModule(CLrep)[(size-1)+a+i-1];
od;
fi;

a:=x_i[2];
Mat_3:=MatricesOfPathAlgebraModule(CLrep)[2*(size-1)+a];
a:=x_i[2];
b:=x_j[2];
Mat_4:=IdentityMat(dim_xi,field);
if a<b then
for i in [1..b-a] do
Mat_4:=Mat_4*MatricesOfPathAlgebraModule(CLrep)[a+i-1];
od;
fi;
...
ChangeCLtoTypeAForDownRow:=function(CLrep,CLIntervalrep,ZigzagPathAlgebra,field)
...
ChangeCLtoTypeAForUpRow:=function(CLrep,CLIntervalrep,ZigzagPathAlgebra,field)
local a,b,i,listSo,listSink,size,lst,
...
ChangeCLtoTypeA:=function(CLrep,CLIntervalrep,ZigzagPathAlgebra,field)
if  CheckKindOfCLInterval(CLIntervalrep)= "2Rows" then
return ChangeCLtoTypeAForTwoRows(CLrep,CLIntervalrep,ZigzagPathAlgebra,field);
elif CheckKindOfCLInterval(CLIntervalrep)= "1RowDown" then
return ChangeCLtoTypeAForDownRow(CLrep,CLIntervalrep,ZigzagPathAlgebra,field);
elif CheckKindOfCLInterval(CLIntervalrep)= "1RowUp" then
return ChangeCLtoTypeAForUpRow(CLrep,CLIntervalrep,ZigzagPathAlgebra,field);
fi;
end;
...
CompressedMultiplicityVer2:=function(CLrep,CLIntervalrep)
local RCLrep,RCLIntervalrep,Q,ZigzagPathAlgebra,field;;
field := LeftActingDomain(CLrep);
Q := Quiver(5,[[2,1,"a"],[2,3,"b"],[4,3,"c"],[4,5,"d"]]);
ZigzagPathAlgebra := PathAlgebra(field, Q);
RCLrep:=ChangeCLtoTypeA(CLrep,CLIntervalrep,ZigzagPathAlgebra,field);
RCLIntervalrep:=ChangeCLtoTypeA(CLIntervalrep,CLIntervalrep,ZigzagPathAlgebra,field);
